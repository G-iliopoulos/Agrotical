...
export default function LoginPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      setLoading(false);
      if (result.success) { const user = users.find(u => u.email === email); redirectByRole(user?.role); }
      else setError(result.message);
    }, 600);
  };

  const handleRoleSelect = (acc: typeof demoAccounts[0]) => { setShowRoles(false); loginAs(acc.id); redirectByRole(acc.role); };
    ...
    <div className="min-h-screen flex">
      <div className="hidden lg:flex lg:w-1/2 relative flex-col justify-between p-12 text-white overflow-hidden"
        ...
        <div className="relative z-10">
          ...
          <div>
            <h2 className="text-4xl text-white mb-4" style={{ fontWeight: 700, lineHeight: 1.2 }}>Οργανώστε την αγροτική σας χρονιά</h2>
            <p className="text-green-200 text-lg leading-relaxed">Καταχωρίστε τα χωράφια σας, εκτιμήστε παραγωγή και κέρδη, και λάβετε εξειδικευμένες συστάσεις από γεωπόνο.</p>
          </div>
        </div>
        <div className="relative z-10 flex items-center gap-2 text-green-300 text-sm">
          <Leaf className="w-4 h-4" /><span>Agrotical © 2026 — Αγροτική Ψηφιακή Πλατφόρμα</span>
        </div>
      </div>
        ...
        <div className="w-full max-w-md">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
                ...
                {showRoles && (
                  <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden z-20">
                    <div className="px-4 py-2.5 border-b border-gray-50 flex items-center justify-between">
                      ...
                      <button onClick={() => setShowRoles(false)} className="p-1 hover:bg-gray-100 rounded-lg"><X className="w-3.5 h-3.5 text-gray-400" /></button>
                    </div>
                    <div className="p-2 space-y-1">
                      {demoAccounts.map(acc => (
                        <button key={acc.id} onClick={() => handleRoleSelect(acc)} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-gray-50 transition-all text-left group">
                          <div className="w-9 h-9 rounded-xl flex items-center justify-center text-lg flex-shrink-0" style={{ background: acc.bg, border: `1px solid ${acc.border}` }}>{acc.icon}</div>
                    ...
                    <div className="px-4 py-2 border-t border-gray-50"><p className="text-xs text-gray-400">Για δοκιμαστικούς σκοπούς — χωρίς εγγραφή</p></div>
                  </div>
                )}
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {error && <div className="p-3 rounded-xl bg-red-50 border border-red-200 text-red-600 text-sm flex items-center gap-2"><span>⚠️</span> {error}</div>}
              <button type="submit" disabled={loading} className="w-full py-3 rounded-xl text-white text-sm transition-all hover:shadow-lg hover:opacity-95 active:scale-95 disabled:opacity-60 mt-1"
                style={{ background: 'linear-gradient(135deg, #2d6a4f, #40916c)', fontWeight: 600 }}>
                {loading ? <span className="flex items-center justify-center gap-2"><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Σύνδεση...</span> : 'Σύνδεση'}
              </button>
            </form>
          </div>
