...
export default function Layout({ children }: LayoutProps) {
  ...
  const getPageTitle = () => { const current = navItems.find(item => location.pathname.startsWith(item.path)); return current?.label ?? 'Agrotical'; };

  const SidebarContent = () => (
    <div className="flex flex-col h-full" style={{ background: theme.bg }}>
      <div className="p-5 border-b border-white/10">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 bg-white/15 rounded-xl flex items-center justify-center"><Sprout className="w-5 h-5 text-white" /></div>
          {sidebarOpen && <div><div className="text-white text-base" style={{ fontWeight: 700 }}>Agrotical</div><div className="text-white/50 text-xs">{getRoleLabel(currentUser.role)}</div></div>}
        </div>
      </div>
        ...
        {navItems.map(item => {
          ...
          return (
            <NavLink key={item.path} to={item.path} onClick={() => setMobileOpen(false)}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all group relative ${isActive ? 'text-white' : 'text-white/60 hover:text-white hover:bg-white/10'}`}
              ...
              {sidebarOpen && (<>
                ...
                {item.badge && item.badge > 0 ? <span className="px-1.5 py-0.5 rounded-full text-xs text-white" style={{ background: '#ef4444', fontWeight: 700, minWidth: '20px', textAlign: 'center' }}>{item.badge}</span> : null}
              </>)}
              {!sidebarOpen && item.badge && item.badge > 0 ? <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full text-xs text-white flex items-center justify-center" style={{ background: '#ef4444', fontSize: '10px', fontWeight: 700 }}>{item.badge}</span> : null}
            </NavLink>
          );
        })}
      </nav>
      <div className="p-3 border-t border-white/10">
        <div className={`flex items-center gap-3 px-3 py-2.5 rounded-xl bg-white/10 ${sidebarOpen ? '' : 'justify-center'}`}>
          <div className="w-8 h-8 rounded-lg flex items-center justify-center text-lg flex-shrink-0 bg-white/20">{getRoleIcon(currentUser.role)}</div>
          {sidebarOpen && <div className="flex-1 min-w-0"><div className="text-white text-sm truncate" style={{ fontWeight: 600 }}>{currentUser.name}</div><div className="text-white/50 text-xs truncate">{currentUser.email}</div></div>}
        </div>
        <button onClick={handleLogout} className={`mt-2 flex items-center gap-2 px-3 py-2 rounded-xl text-white/60 hover:text-white hover:bg-white/10 transition-all w-full ${sidebarOpen ? '' : 'justify-center'}`}>
          <LogOut className="w-4 h-4" />{sidebarOpen && <span className="text-sm">Αποσύνδεση</span>}
        </button>
      </div>
    </div>
  );
    ...
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      ...
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 z-50 flex">
          ...
          <aside className="relative w-64 flex flex-col"><SidebarContent /></aside>
        </div>
      )}
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="bg-white border-b border-gray-200 px-6 py-4 flex items-center gap-4">
          <button onClick={() => { setSidebarOpen(!sidebarOpen); setMobileOpen(!mobileOpen); }} className="p-2 rounded-lg hover:bg-gray-100 transition-colors">
            <Menu className="w-5 h-5 text-gray-600" />
          </button>
          <div className="flex-1"><h1 className="text-lg" style={{ fontWeight: 600, color: '#111827' }}>{getPageTitle()}</h1></div>
          <div className="flex items-center gap-3">
            ...
            <div className="relative">
              <button onClick={() => setShowUserMenu(!showUserMenu)} className="flex items-center gap-2 p-2 rounded-lg hover:bg-gray-100 transition-colors">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center text-base" style={{ background: theme.light }}>{getRoleIcon(currentUser.role)}</div>
              ...
              {showUserMenu && (
                <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-xl shadow-lg border border-gray-100 py-2 z-50">
                  <div className="px-4 py-3 border-b border-gray-100">
                    ...
                    <div className="mt-1 inline-flex px-2 py-0.5 rounded-full text-xs" style={{ background: theme.light, color: theme.accent, fontWeight: 600 }}>{getRoleLabel(currentUser.role)}</div>
                  </div>
